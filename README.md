...
The universal presentation is in `shared/styles.css` and `shared/display-template.css`; the build copies these byte-for-byte into the static bundle. To synchronize from another canonical template checkout, pass `--shared-source DIRECTORY` pointing to its CSS directory. This is an explicit two-file copy, not a project import. The hardware route uses its own `voice-device-rows.js` and `voice-device-rows.css`: one item per full-width row, image/name/purchase on the left and components on the right; below 600px these stack. The 13 procurement records join the 36-item catalog by ID, without duplicate cards. Four explicit categories distinguish voice candidates, DIY donors and design references. Shared geometry remains unchanged for other consumers. Legacy renderer assets remain available for cached older HTML; deploy the new assets before atomically replacing the index. Feature statuses are explicit package-evidence mappings, never inferred from chip capabilities.
